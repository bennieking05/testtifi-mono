...
-- 1) Add nullable columns to Purchase so existing rows remain valid
ALTER TABLE `Purchase`
  ADD COLUMN IF NOT EXISTS `amountCents` INT NULL,
  ADD COLUMN IF NOT EXISTS `creditsAdded` INT NULL,
  ADD COLUMN IF NOT EXISTS `stripePaymentIntentId` VARCHAR(191) NULL,
