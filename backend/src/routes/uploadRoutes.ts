...
/* ═══════════════════════════════════════════════════════════════════════════
 * TWO-STEP SIGNED-URL UPLOAD  (bypasses Cloud Run 32 MiB HTTP/1 limit)
 *
 *   Step 1  POST /init   → reserves credit, returns a GCS signed PUT URL
 *   Step 2  POST /complete → browser already uploaded to GCS; creates DB records
 * ═══════════════════════════════════════════════════════════════════════════ */
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    const userId = (req as any).user?.userId as string | undefined;
    if (!userId) { res.status(401).json({ error: "Missing authentication" }); return; }
    ...
    if (!ALLOWED_EXTENSIONS.has(ext)) {
      res.status(400).json({ error: "Unsupported file type. Allowed: PDF, DOC, DOCX, TXT" });
      return;
    }
    ...
    try {
      ...
      if (debitError instanceof InsufficientCreditsError) {
        res.status(402).json({ error: "Insufficient credits. Please purchase more credits to create a summary.", required: debitError.required, available: debitError.available });
        return;
      }
      console.error("[upload/init] credit error:", debitError);
      res.status(500).json({ error: "Failed to reserve credits" });
      return;
    }

    // Generate a unique object key & signed URL
    const safeName = safeBaseName(fileName);
    const objectKey = `${userId}/${reservationId}-${safeName}`;
    const gcsFile = depositionBkt.file(objectKey);
    ...
    try {
      ...
      console.error("[upload/init] signed URL error:", err);
      // Refund credit on failure
      try { await refundCreditsForSummary(userId, reservationId); } catch { /* ignore */ }
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const { reservationId, objectKey, fileName, summaryName, deponent, notifyOnComplete } = req.body || {};
    if (!reservationId || !objectKey || !fileName) {
      res.status(400).json({ error: "reservationId, objectKey, and fileName are required" });
      return;
    }

    // Verify the GCS object actually exists
    const gcsFile = depositionBkt.file(objectKey);
    const [exists] = await gcsFile.exists();
    if (!exists) {
      // Refund the credit – the browser never finished the upload
      try { await refundCreditsForSummary(userId, reservationId); } catch { /* ignore */ }
    ...
    try {
      ...
      // Update ledger entry description
      try {
        await prisma.ledgerEntry.updateMany({
          where: { idempotencyKey: `summary:${reservationId}` },
          data: { description: summaryName || fileName },
        });
      } catch { /* ignore */ }
